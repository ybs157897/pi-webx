/**
 * LobeChat-style session list: one search box over two groups — live pi sessions
 * owned by the bridge, and resumable sessions discovered on disk, the latter split
 * into date buckets (今天 / 昨天 / 近 7 天 / 更早) with a per-row action menu.
 *
 * Purely presentational. Every datum arrives through props; searching is a local
 * filter over the display title, model and cwd, and the only local state is the
 * query, the pending renames and the row menus.
 */
...
/** Lighter than `GroupHeader`: stored sessions are already nested one level deep. */
...
function LiveRow({
  ...
  const model = [session.provider, session.model]
    ...
    .join(' · ');
  const subtitle = [model, session.alive ? '' : '已结束']
...
function StoredRow({
  ...
  const title = session.preview ? truncate(session.preview, 60) : session.id.slice(0, 12);
...
export interface SessionListProps {
  /** live pi sessions owned by the bridge */
  sessions: SessionSummary[];
  /** past sessions discovered on disk, already filtered to the current workspace */
  stored: StoredSession[];
  storedLoading?: boolean;
  activeId: string | null;
  /** home dir, used to abbreviate /Users/x/... to ~/... */
  home?: string;
  onSwitch: (id: string) => void;
  onKill: (id: string) => void;
  onResume: (session: StoredSession) => void;
  /** renames the live session via pi's set_session_name */
  onRename: (id: string, name: string) => void;
  onRefreshStored: () => void;
  /** permanently deletes a persisted transcript; the parent refreshes afterwards */
...
export function SessionList({
  ...
  const visibleSessions = useMemo(
    () =>
      sessions.filter((session) => matchesQuery(needle, [titleOf(session), session.model, session.cwd])),
    [needle, sessions, titleOf],
  );
  const visibleStored = useMemo(
    () => stored.filter((session) => matchesQuery(needle, [session.preview, session.cwd])),
    [needle, stored],
  );

  /** Search-filtered stored sessions, split into 今天/昨天/近 7 天/更早 sub-groups. */
  ...
  const requestDeleteStored = useCallback(
    (session: StoredSession) => {
      const label = session.preview ? truncate(session.preview, 40) : session.id.slice(0, 12);
      confirm({
        title: '从磁盘删除会话',
        content: `确定要删除「${label}」吗？该会话的转录文件会从磁盘永久删除，无法恢复。`,
        okText: '删除',
        okType: 'danger',
        cancelText: '取消',
        centered: true,
    ...
    <Flexbox style={{ height: '100%', minHeight: 0 }}>
      <Flexbox paddingInline={12} paddingBlock={6} gap={6} style={{ flexShrink: 0 }}>
        <Flexbox horizontal align="center" gap={6}>
          ...
          {needle.length > 0 && <CountBadge count={matchCount} />}
        </Flexbox>
      </Flexbox>

      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', paddingInline: 12, paddingBottom: 12 }}>
        {nothingAtAll || nothingMatched ? (
          <Flexbox align="center" justify="center" style={{ paddingBlock: 32 }}>
            <Empty
              image={Empty.PRESENTED_IMAGE_SIMPLE}
              description={nothingMatched ? '没有匹配的会话' : '暂无会话'}
            />
          </Flexbox>
        ) : (
          <Flexbox gap={2}>
            <GroupHeader label="运行中的会话" count={visibleSessions.length} />
            ...
            {visibleStored.length === 0 ? (
              <EmptyHint text={storedLoading === true ? '加载中…' : '暂无'} />
            ) : (
              storedBuckets.map((bucket) => (
                <Flexbox key={bucket.key}>
                  <BucketHeader label={bucket.label} count={bucket.items.length} />
                  {bucket.items.map((session) => (
                    <StoredRow
                      key={session.path}
                      session={session}
                      onResume={onResume}
                      onDelete={onDeleteStored === undefined ? undefined : requestDeleteStored}
                    />
                  ))}
                </Flexbox>
              ))
...
/**
 * Usage assert: the sidebar renders `<SessionList {...props} />` with exactly the
 * props below, so the assignment fails `tsc` the moment the component stops
 * accepting `SessionListProps`.
 */
type _Assert = SessionListProps;
const _assertComponent: FC<_Assert> = SessionList;

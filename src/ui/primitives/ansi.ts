...
// (`ansi.ts`): the terminal card renders a shell command's output as a terminal
// would, which needs a real SGR parser plus a replay of the cursor movements
// every progress bar and spinner writes. Hand-rolling a narrower version would
// misalign exactly the column-aligned output this exists for.
//
...
const WIDE_CHAR = new RegExp(
  ...
  // width — `\u2713` (the check every progress line writes) advances one column,
  // verified against a real terminal, so taking the whole block as wide
  // misaligned exactly the output this card exists for.

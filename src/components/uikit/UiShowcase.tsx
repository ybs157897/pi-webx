...
import { App as AntApp, Segmented, theme } from 'antd';
      ...
      children: [
        {
          ...
          series: [{ name: '营收（千元）', data: [32, 45, 38, 61, 55, 72, 68] }],
        },
      ],
    },
    {
...
type ViewMode = 'ours' | 'compare' | 'tokui';

/** Gallery proving every component kind renders — with a TokUI side-by-side. */
    ...
    <Flexbox gap={20}>
      ...
      {SECTIONS.map((section) => (
        <Flexbox key={section.name} gap={10}>
          ...
          {mode === 'compare' ? (
            <Flexbox gap={10} direction="vertical">
              <Flexbox gap={8}>
                <Text fontSize={11} weight={600} style={{ color: token.colorTextQuaternary }}>
                  自研渲染
                </Text>
              ...
              <Flexbox gap={8}>
                ...
                <div style={{ border: `1px solid ${token.colorBorderSecondary}`, borderRadius: token.borderRadiusLG, padding: 12 }}>
                  <TokUIView dsl={specToTokDsl(section.spec)} theme={themeMode} onAction={handled} />
                </div>
              </Flexbox>
            </Flexbox>
          ) : mode === 'tokui' ? (

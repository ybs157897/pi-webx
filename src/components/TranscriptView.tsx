...
function EntryView({
  ...
  switch (entry.kind) {
    ...
    case 'assistant':
      return <AssistantMessageItem entry={entry} onAction={onAction} renderStyle={renderStyle} />;
      ...
      >
        <Flexbox gap={20} paddingInline={20} paddingBlock={20} style={{ maxWidth: 900, margin: '0 auto' }}>
          {transcript.entries.map((entry) => (
            <EntryView key={entry.id} entry={entry} onAction={onAction} renderStyle={renderStyle} />
          ))}
